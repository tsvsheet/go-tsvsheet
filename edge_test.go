...
func TestCompute_MixedComparisonAndArithmetic(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrValue), cellAt(t, g, 0, 1)) // string < number
	assert.Equal(t, string(tsvsheet.ErrValue), cellAt(t, g, 0, 2)) // 1 + string
...
func TestCompute_EmptyAggregates(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrValue), cellAt(t, g, 0, 2)) // min of nothing
	assert.Equal(t, string(tsvsheet.ErrValue), cellAt(t, g, 0, 3)) // max of nothing
	assert.Equal(t, string(tsvsheet.ErrDiv), cellAt(t, g, 0, 4))   // avg of nothing
}
...
func TestCompute_Arity(t *testing.T) {
	...
	cases := map[string]string{
		"abs(A1, C1)":       string(tsvsheet.ErrValue),
		"len(A1, C1)":       string(tsvsheet.ErrValue),
		"round()":           string(tsvsheet.ErrValue),
		"round(A1, C1, D1)": string(tsvsheet.ErrValue),
		"if(A1)":            string(tsvsheet.ErrValue),
		"bogus(A1)":         string(tsvsheet.ErrName),
		"round(A1, Z99)":    string(tsvsheet.ErrRef), // error place argument
	}
	for expr, want := range cases {
		t.Run(expr, func(t *testing.T) {
			t.Parallel()
...
func TestCompute_RowZeroIsRef(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "A0"))
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "sum(A0:A0)"))
}
...
func TestCompute_RangeInScalarContext(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrValue), formula1(t, "A2:C2 + 1"))
}
...
func TestCompute_BuiltinsPropagateErrors(t *testing.T) {
	...
	for _, expr := range []string{
		...
		t.Run(expr, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, expr))
		})
	}
}
...
func TestCompute_StringLeftArithmetic(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrValue), cellAt(t, g, 0, 1))
}
