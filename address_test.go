...
func TestParseAddress(t *testing.T) {
	...
	cases := map[string]tsvsheet.Address{
		"A1":   {Row: 0, Col: 0},
		"B2":   {Row: 1, Col: 1},
		"Z1":   {Row: 0, Col: 25},
		"AA1":  {Row: 0, Col: 26},
	...
	for src, want := range cases {
		t.Run(src, func(t *testing.T) {
			t.Parallel()
			got, err := tsvsheet.ParseAddress(tsvsheet.AddressText(src))
...
func TestParseAddress_Invalid(t *testing.T) {
	...
	for name, src := range cases {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			_, err := tsvsheet.ParseAddress(tsvsheet.AddressText(src))
			require.Error(t, err)
...
func TestAddress_RoundTrip(t *testing.T) {
	...
	for _, s := range []string{"A1", "Z26", "AA100", "ZZ1", "AAA5"} {
		addr, err := tsvsheet.ParseAddress(tsvsheet.AddressText(s))
		require.NoError(t, err)
