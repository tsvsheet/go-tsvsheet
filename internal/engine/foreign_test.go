...
func TestForeign_ErrorModes(t *testing.T) {
	...
	cases := map[string]string{
		"=\"missing.tsvt\"!A1":               string(engine.ErrRef), // single, unresolved
		"=sum(\"missing.tsvt\"!A1:A3)":       string(engine.ErrRef), // range, unresolved
		"=index(\"missing.tsvt\"!A1:B2,1,1)": string(engine.ErrRef), // matrix, unresolved
	}
