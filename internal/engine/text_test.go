...
func TestText_ErrorsAndEdges(t *testing.T) {
	...
	cases := map[string]string{
		...
		`regexextract("abc", "[0-9]+")`: string(engine.ErrNA), // no match
...
func TestText_ReptLazyDispatch(t *testing.T) {
	...
	assert.Equal(t, string(engine.ErrValue), formula1(t, `rept("a")`))       // wrong arity → #VALUE!
	assert.Equal(t, string(engine.ErrValue), formula1(t, `rept("a", 1, 2)`)) // wrong arity → #VALUE!
	assert.Equal(t, string(engine.ErrDiv), formula1(t, "rept(1/0, 2)"))      // an argument error propagates
}
