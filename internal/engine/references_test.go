...
func addr(row, col int) engine.Address { return engine.Address{Row: row, Col: col} }
...
func TestPrecedents_SingleAndRange(t *testing.T) {
	...
	assert.Equal(t, engine.Span{From: addr(0, 0), To: addr(0, 0)}, spans[0]) // A1
	assert.Equal(t, engine.Span{From: addr(0, 0), To: addr(1, 1)}, spans[1]) // A1:B2
}
...
func TestDependents_ReverseEdge(t *testing.T) {
	...
	assert.Equal(t, []engine.Address{addr(0, 1), addr(0, 2)}, s.Dependents(addr(0, 0)))
	assert.Equal(t, []engine.Address{addr(0, 2)}, s.Dependents(addr(1, 0)))
}
