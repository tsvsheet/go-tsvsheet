...
func embedGrid(t *testing.T, root string, sheets map[string]string) tsvsheet.Grid {
	t.Helper()
	s, err := tsvsheet.Parse([]byte(root))
	require.NoError(t, err)
	return s.ComputeWith(tsvsheet.ComputeOptions{Loader: memLoader(sheets), Base: "root"})
}
...
func TestEmbed_FailureModes(t *testing.T) {
	...
	cases := map[string]string{
		"=sheet()":            string(tsvsheet.ErrValue), // SHEET arity
		"=sheet(1/0)":         string(tsvsheet.ErrDiv),   // path expression errors through
		"=sheet(\"missing\")": string(tsvsheet.ErrRef),   // loader cannot resolve
		"=sheet(\"noout\")":   string(tsvsheet.ErrRef),   // no OUTPUT cell
		"=sheet(\"twoout\")":  string(tsvsheet.ErrRef),   // ambiguous OUTPUT
		"=sheet(\"needs\")":   string(tsvsheet.ErrRef),   // INPUT(1) with no args
		"=sheet(\"badidx\")":  string(tsvsheet.ErrValue), // INPUT with a text index
	}
...
func TestEmbed_CheckAcceptsBuiltins(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("=sheet(\"x\", input(1))\t=output(A1)\n"))
	require.NoError(t, err)
	assert.Empty(t, tsvsheet.Check(s))
}
...
func TestEmbeddedGrid_ReturnsSubSheet(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("=sheet(\"child\", 5)\n"))
	require.NoError(t, err)
	path, grid, ok := s.EmbeddedGrid(addr(0, 0), tsvsheet.ComputeOptions{
		Loader: memLoader(map[string]string{"child": "=input(1)\t=output(A1 * 2)\n"}),
		Base:   "root",
	})
	require.True(t, ok)
	assert.Equal(t, tsvsheet.Path("child"), path)
...
func TestEmbeddedGrid_NotAnEmbed(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("hi\t=A1\n"))
	require.NoError(t, err)

	_, _, litOK := s.EmbeddedGrid(addr(0, 0), tsvsheet.ComputeOptions{})
	assert.False(t, litOK) // a literal
	_, _, refOK := s.EmbeddedGrid(addr(0, 1), tsvsheet.ComputeOptions{})
	assert.False(t, refOK) // a formula, but not a SHEET call
	_, _, offOK := s.EmbeddedGrid(addr(9, 9), tsvsheet.ComputeOptions{})
...
func TestEmbeddedGrid_UnresolvedIsNotOK(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("=sheet(\"missing\")\n"))
	require.NoError(t, err)
	_, _, ok := s.EmbeddedGrid(addr(0, 0), tsvsheet.ComputeOptions{
