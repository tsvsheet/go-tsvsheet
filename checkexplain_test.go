...
func parse(t *testing.T, src string) tsvsheet.Sheet {
	t.Helper()
...
func TestCheck_Clean(t *testing.T) {
	...
	assert.Empty(t, tsvsheet.Check(parse(t, "1\t2\t=A1 + B1\n")))
}

func TestCheck_UnknownFunction(t *testing.T) {
	t.Parallel()

	diags := tsvsheet.Check(parse(t, "1\t=bogus(A1)\n"))
...
func TestCheck_NumberFormulaHasNoRefs(t *testing.T) {
	...
	assert.Empty(t, tsvsheet.Check(parse(t, "=1 + 2\n")))
}
...
func TestCheck_KnownFunctionsClean(t *testing.T) {
	...
	assert.Empty(
		t,
		tsvsheet.Check(parse(t, "1\t2\t=if(isnumber(A1), countif(unique(A1:B1), 1), index(A1:B1, 1, 1))\n")),
	)
}
...
func TestCheck_WalksIntoUnaryPercentBinaryAndCall(t *testing.T) {
	...
	for _, src := range []string{"=-bogus(A1)", "=bogus(A1)%", "=bogus(A1) + 1", "=abs(bogus(A1))"} {
		t.Run(src, func(t *testing.T) {
			t.Parallel()
			diags := tsvsheet.Check(parse(t, "1\t"+src+"\n"))
			require.Len(t, diags, 1)
			assert.Contains(t, diags[0].Message, "bogus")
		})
	}
...
func TestExplain_Formula(t *testing.T) {
	...
	trace, err := tsvsheet.Explain(parse(t, "2\t3\t=A1 + B1\n"), tsvsheet.Address{Row: 0, Col: 2})
	require.NoError(t, err)
	assert.Equal(t, "C1", trace.Cell)
	assert.Equal(t, "5", trace.Value)
	assert.Equal(t, "A1 + B1", trace.Formula)
	assert.Equal(t, []tsvsheet.TraceInput{{Ref: "A1", Value: "2"}, {Ref: "B1", Value: "3"}}, trace.Inputs)
}
...
func TestExplain_RangeInput(t *testing.T) {
	...
	trace, err := tsvsheet.Explain(parse(t, "1\t2\t=sum(A1:B1)\n"), tsvsheet.Address{Row: 0, Col: 2})
	require.NoError(t, err)
...
func TestExplain_Literal(t *testing.T) {
	...
	trace, err := tsvsheet.Explain(parse(t, "hello\t=A1\n"), tsvsheet.Address{Row: 0, Col: 0})
	require.NoError(t, err)
...
func TestExplain_OutOfGrid(t *testing.T) {
	...
	_, err := tsvsheet.Explain(parse(t, "1\t2\n"), tsvsheet.Address{Row: 9, Col: 9})
	require.Error(t, err)
...
func TestExplain_RendersEveryExpressionForm(t *testing.T) {
	...
	for src, want := range cases {
		t.Run(src, func(t *testing.T) {
			t.Parallel()
			trace, err := tsvsheet.Explain(parse(t, "5\t"+src+"\n"), tsvsheet.Address{Row: 0, Col: 1})
			require.NoError(t, err)
