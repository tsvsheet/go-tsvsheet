...
func TestDate_TodayNowInjectedClock(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("=today()\t=now()\t=if(today(), 1, 0)\n"))
...
func TestDate_ClockArity(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrValue), formula1(t, "today(5)")) // no arguments allowed
	assert.Equal(t, string(tsvsheet.ErrValue), formula1(t, "now(5)"))
}
