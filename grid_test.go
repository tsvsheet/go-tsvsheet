...
func TestReadTSV(t *testing.T) {
	...
	g, err := tsvsheet.ReadTSV(strings.NewReader("a\tb\n1\t2\n"))
	require.NoError(t, err)
	assert.Equal(t, tsvsheet.Grid{{"a", "b"}, {"1", "2"}}, g)
}
...
func TestReadTSV_SkipsComments(t *testing.T) {
	...
	g, err := tsvsheet.ReadTSV(strings.NewReader(
		"#!/usr/bin/env tsvsheet\n# a note\na\tb\n# mid\n#N/A\t=A2\n",
	))
	require.NoError(t, err)
	assert.Equal(t, tsvsheet.Grid{{"a", "b"}, {"#N/A", "=A2"}}, g)
}
...
func TestReadTSV_CommentOrDataOnFirstLine(t *testing.T) {
	...
	comment, err := tsvsheet.ReadTSV(strings.NewReader("# header\nx\ty\n"))
	require.NoError(t, err)
	assert.Equal(t, tsvsheet.Grid{{"x", "y"}}, comment)

	data, err := tsvsheet.ReadTSV(strings.NewReader("x\ty\n"))
	require.NoError(t, err)
	assert.Equal(t, tsvsheet.Grid{{"x", "y"}}, data)
}

func TestReadTSV_Ragged(t *testing.T) {
	t.Parallel()

	g, err := tsvsheet.ReadTSV(strings.NewReader("a\tb\tc\n1\n"))
	require.NoError(t, err)
	assert.Equal(t, tsvsheet.Grid{{"a", "b", "c"}, {"1"}}, g)
}
...
func TestReadTSV_Error(t *testing.T) {
	...
	_, err := tsvsheet.ReadTSV(failingReader{})
	require.Error(t, err)
...
func TestWriteTSV(t *testing.T) {
	...
	require.NoError(t, tsvsheet.WriteTSV(&b, tsvsheet.Grid{{"a", "b"}, {"1", "2"}}))
	assert.Equal(t, "a\tb\n1\t2\n", b.String())
}
...
func TestWriteTSV_Error(t *testing.T) {
	...
	err := tsvsheet.WriteTSV(failingWriter{}, tsvsheet.Grid{{"a"}})
	require.Error(t, err)
...
func TestReadWriteRoundTrip(t *testing.T) {
	...
	const in = "1\t2\t3\n4\t5\t6\n"
	g, err := tsvsheet.ReadTSV(strings.NewReader(in))
	require.NoError(t, err)
