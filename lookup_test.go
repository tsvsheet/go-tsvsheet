...
func TestLookup_VlookupIndexMatch(t *testing.T) {
	...
	cases := map[string]string{
		...
		`vlookup("Zoe", A1:B3, 2)`: string(tsvsheet.ErrNA),  // not found
		`vlookup("Bob", A1:B3, 9)`: string(tsvsheet.ErrRef), // column out of range
		`index(A1:B3, 2, 2)`:       "72",
		`index(A1:B3, 2)`:          "Bob", // column defaults to 1
		`index(A1:B3, 9, 1)`:       string(tsvsheet.ErrRef),
		`index(A1, 1, 1)`:          "Alice", // single-cell range
		`index(5, 1, 1)`:           "5",     // scalar becomes a 1x1
		`match("Carol", A1:A3)`:    "3",
		`match("Zoe", A1:A3)`:      string(tsvsheet.ErrNA),
		`rows(A1:B3)`:              "3",
		`columns(A1:B3)`:           "2",
		`choose(2, "x", "y", "z")`: "y",
		`index(A0:A0, 1, 1)`:       string(tsvsheet.ErrRef), // off-grid range
	}
...
func TestLookup_Hlookup(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrNA), cellAt(t, g, 2, 1))  // not found
	assert.Equal(t, string(tsvsheet.ErrRef), cellAt(t, g, 2, 2)) // row out of range
}
...
func TestLookup_ArityAndArgErrors(t *testing.T) {
	...
	cases := map[string]string{
		`rows(A1:B3, 1)`:   string(tsvsheet.ErrValue), // rows takes one arg
		`index(A1:B3)`:     string(tsvsheet.ErrValue), // too few
		`match("x")`:       string(tsvsheet.ErrValue), // too few
		`vlookup("x", A1)`: string(tsvsheet.ErrValue), // too few
		`choose(0, "a")`:   string(tsvsheet.ErrValue), // index below 1
		`choose(9, "a")`:   string(tsvsheet.ErrValue), // index past end
	}
	for expr, want := range cases {
		t.Run(expr, func(t *testing.T) {
			t.Parallel()
