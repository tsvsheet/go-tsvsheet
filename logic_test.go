...
func TestLogic_BooleansAndConditionals(t *testing.T) {
	...
	cases := map[string]string{
		...
		"iferror(D1, 7)":     "4",
		"ifna(na(), 5)":      "5",
		"ifna(D1 / 0, 5)":    string(tsvsheet.ErrDiv), // IFNA does not catch #DIV/0!
		"ifna(D1, 5)":        "4",
...
func TestLogic_ConditionalArityAndErrors(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrValue), formula1(t, "ifs(TRUE)"))          // odd count
	assert.Equal(t, string(tsvsheet.ErrNA), formula1(t, "ifs(FALSE, 1)"))         // no truthy
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "ifs(Z99, 1)"))          // error condition
	assert.Equal(t, string(tsvsheet.ErrValue), formula1(t, "iferror(1)"))         // wrong arity
	assert.Equal(t, string(tsvsheet.ErrValue), formula1(t, "switch(1, 2)"))       // too few
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "switch(Z99, 1, 2)"))    // error subject
	assert.Equal(t, string(tsvsheet.ErrNA), formula1(t, "switch(1, 2, 3, 4, 5)")) // no match, no default
	assert.Equal(t, string(tsvsheet.ErrValue), formula1(t, "isnumber(1, 2)"))     // inspector arity
}
...
func TestLogic_Parity(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "iseven(Z99)"))         // error propagates
...
func TestLogic_Inspectors(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "n(Z99)"))   // error propagates
