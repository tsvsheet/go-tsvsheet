...
func TestArray_SpillBlocked(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrSpill), cellAt(t, g, 0, 0))
...
func TestArray_Errors(t *testing.T) {
	...
	cases := map[string]string{
		"sequence()":          string(tsvsheet.ErrValue), // arity
		"sequence(0)":         string(tsvsheet.ErrValue), // rows < 1
		"sequence(1, 0)":      string(tsvsheet.ErrValue), // cols < 1
		"sequence(6000000)":   string(tsvsheet.ErrValue), // exceeds the default cell budget (OOM guard)
		"sequence(3000,3000)": string(tsvsheet.ErrValue), // rows×cols (9M) exceeds the budget
		"transpose(A1, A2)":   string(tsvsheet.ErrValue), // arity
		"unique(A1, A2)":      string(tsvsheet.ErrValue),
		"sort(A1, A2)":        string(tsvsheet.ErrValue),
		"filter(A1)":          string(tsvsheet.ErrValue),
		"flatten(A1, A2)":     string(tsvsheet.ErrValue),
	}
	...
	assert.Equal(t, string(tsvsheet.ErrNA),
