...
func TestCells_ProjectsNonEmpty(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("a\t\t=A1\n"))
...
func TestSet_LiteralInPlace(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("1\t2\n"))
	require.NoError(t, err)

	next, err := s.Set(tsvsheet.Address{Row: 0, Col: 0}, "9", tsvsheet.DefaultLimits())
	require.NoError(t, err)
...
func TestSet_FormulaComputes(t *testing.T) {
	...
	s, err := tsvsheet.Parse([]byte("2\t3\n"))
	require.NoError(t, err)

	next, err := s.Set(tsvsheet.Address{Row: 0, Col: 1}, "=A1*10", tsvsheet.DefaultLimits())
	require.NoError(t, err)
...
func TestSet_GrowsGrid(t *testing.T) {
	...
	next, err := s.Set(tsvsheet.Address{Row: 2, Col: 3}, "x", tsvsheet.DefaultLimits())
	require.NoError(t, err)
...
func TestSet_MalformedFormulaIsSyntaxError(t *testing.T) {
	...
	_, err = s.Set(tsvsheet.Address{Row: 0, Col: 0}, "=sum(", tsvsheet.DefaultLimits())
	require.Error(t, err)
...
func TestSet_RejectsNegativeAddress(t *testing.T) {
	...
	_, err = s.Set(tsvsheet.Address{Row: -1, Col: 0}, "x", tsvsheet.DefaultLimits())
	require.Error(t, err)
	assert.ErrorIs(t, err, constants.ErrInvalidValue)

	// An address beyond the grid limit is rejected before growing (OOM guard).
	_, err = s.Set(tsvsheet.Address{Row: 2_000_000, Col: 0}, "x", tsvsheet.DefaultLimits())
	require.Error(t, err)
	assert.ErrorIs(t, err, constants.ErrInvalidValue)
	_, err = s.Set(tsvsheet.Address{Row: 0, Col: 2_000_000}, "x", tsvsheet.DefaultLimits())
	assert.ErrorIs(t, err, constants.ErrInvalidValue)
}
