...
func TestStat_Edges(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrNA), formula1(t, "mode(1, 2, 3)"))   // no repeat
	assert.Equal(t, string(tsvsheet.ErrNum), formula1(t, "geomean(1, -4)")) // non-positive
	assert.Equal(t, string(tsvsheet.ErrNum), formula1(t, "large(1, 2, 9)")) // k out of range
	assert.Equal(t, string(tsvsheet.ErrNum), formula1(t, "small(1, 2, 9)"))
	...
	// → #DIV/0!.
	assert.Equal(t, string(tsvsheet.ErrNum), cellAt(t, compute(t, "\t=median(A1)\n"), 0, 1))
	assert.Equal(t, string(tsvsheet.ErrNum), cellAt(t, compute(t, "\t=geomean(A1)\n"), 0, 1))
	assert.Equal(t, string(tsvsheet.ErrDiv), formula1(t, "stdev(5)")) // sample needs >= 2
}
...
func TestStat_CriteriaEdges(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrDiv), cellAt(t, g, 1, 0))   // no match
	assert.Equal(t, "3", cellAt(t, g, 1, 1))                       // D1+E1 (col3 has no sum cell)
	assert.Equal(t, string(tsvsheet.ErrValue), cellAt(t, g, 1, 2)) // countif arity
	...
	assert.Equal(t, string(tsvsheet.ErrValue), cellAt(t, compute(t, "1\t=sumif(A1)\n"), 0, 1))
	errSum := compute(t, "10\t20\t=1/0\t5\n=sumif(A1:B1, \">5\", C1:D1)\n") // C1 is #DIV/0!
	assert.Equal(t, string(tsvsheet.ErrDiv), cellAt(t, errSum, 1, 0))
}
