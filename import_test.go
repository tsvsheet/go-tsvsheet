...
const (
	mediaSheetWire  tsvsheet.MediaType = "application/vnd.tsvsheet+tsv"
	mediaCellWire   tsvsheet.MediaType = "application/vnd.tsvsheet.cell+tsv"
	mediaRowWire    tsvsheet.MediaType = "application/vnd.tsvsheet.row+tsv"
	mediaColumnWire tsvsheet.MediaType = "application/vnd.tsvsheet.column+tsv"
	mediaRangeWire  tsvsheet.MediaType = "application/vnd.tsvsheet.range+tsv"
)
...
func (f echoFetcher) Fetch(_ tsvsheet.ImportURL, accept tsvsheet.MediaType) (tsvsheet.FetchResult, error) {
	...
	return tsvsheet.FetchResult{Body: f.body, ContentType: accept}, nil
}
...
type fixedFetcher struct {
	err    error
	result tsvsheet.FetchResult
}

func (f fixedFetcher) Fetch(_ tsvsheet.ImportURL, _ tsvsheet.MediaType) (tsvsheet.FetchResult, error) {
	return f.result, f.err
}

// importGrid parses src and computes it with the injected Fetcher and Limits.
func importGrid(t *testing.T, src string, f tsvsheet.Fetcher, limits tsvsheet.Limits) tsvsheet.Grid {
	t.Helper()
	s, err := tsvsheet.Parse([]byte(src))
	require.NoError(t, err)
	return s.ComputeWith(tsvsheet.ComputeOptions{At: time.Now(), Fetcher: f, Limits: limits})
}
...
func TestImportHandshakeMismatch(t *testing.T) {
	...
	f := fixedFetcher{result: tsvsheet.FetchResult{Body: []byte("a\tb\n"), ContentType: mediaCellWire}}
	grid := importGrid(t, "=importrow(\"u\")\n", f, tsvsheet.DefaultLimits())
...
func TestImportShapeMismatches(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			grid := importGrid(t, tc.src, echoFetcher{body: []byte(tc.body)}, tsvsheet.DefaultLimits())
			assert.Equal(t, "#IMPORT!", cellAt(t, grid, 0, 0))
		})
	}
}
...
func TestImportOversizeRejected(t *testing.T) {
	...
	tight := tsvsheet.Limits{ResultCells: 2, GridDim: 20_000, ResultBytes: 64 << 10}
	cases := map[string]struct {
		src  string
		body string
	}{
