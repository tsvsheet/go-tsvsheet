...
func compute(t *testing.T, src string) tsvsheet.Grid {
	t.Helper()
...
func cellAt(t *testing.T, g tsvsheet.Grid, row, col int) string {
	t.Helper()
...
func TestCompute_DivZero(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrDiv), formula1(t, "A1 / 0"))
	assert.Equal(t, string(tsvsheet.ErrDiv), formula1(t, "mod(A1, 0)"))
}
...
func TestCompute_StringAndErrorLiterals(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrNA), formula1(t, "#N/A"))
	assert.Equal(t, string(tsvsheet.ErrNA), formula1(t, "#N/A + 1")) // propagates
}
...
func TestCompute_CircularReference(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrCirc), cellAt(t, g, 0, 0))
	assert.Equal(t, string(tsvsheet.ErrCirc), cellAt(t, g, 0, 1))
}

func TestCompute_SelfReference(t *testing.T) {
	t.Parallel()

	assert.Equal(t, string(tsvsheet.ErrCirc), cellAt(t, compute(t, "=A1 + 1\n"), 0, 0))
}

func TestCompute_OutOfGrid(t *testing.T) {
	t.Parallel()

	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "Z99"))
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "sum(A5:A9)")) // rows past the grid
}

func TestCompute_ErrorPropagation(t *testing.T) {
	t.Parallel()

	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "Z99 + A1"))        // left
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "A1 + Z99"))        // right
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "-Z99"))            // unary
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "sum(Z99)"))        // aggregate
	assert.Equal(t, string(tsvsheet.ErrRef), formula1(t, "if(Z99, A1, C1)")) // condition
}
...
func TestCompute_ErrorLiteralPropagates(t *testing.T) {
	...
	assert.Equal(t, string(tsvsheet.ErrRef), cellAt(t, g, 0, 1))
}
